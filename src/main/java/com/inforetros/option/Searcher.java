...
import static com.inforetros.command.SporchCommand.NUMOFRESULTS;

public class Searcher {
    private String fieldName;
    private String qstr;

    public Searcher(String fieldName, String qstr) {
        ...
        try {
            ...
            if(fieldName.equalsIgnoreCase("all")) {
                parser = new MultiFieldQueryParser(
                        new String[] {"track_name","album_name","artist_name"},
                        analyzer);
            }
            else {
                parser = new QueryParser( fieldName, analyzer ); // a query parser that transforms a text string into Lucene's query object
            }

            Query query = parser.parse( qstr ); // this is Lucene's query object

            // Okay, now let's open an index and search for documents
            Directory dir = FSDirectory.open( new File(Sporch.INDEX_PATH).toPath() );
            ...
            for ( ScoreDoc scoreDoc : docs.scoreDocs ) {
                ...
                String playlist_name = LuceneUtils.getDocno( searcher, "playlist_name", docid );
                //String track_name = LuceneUtils.getDocno( index, "track_name", docid );
                String track_name = LuceneUtils.getDocno( searcher, "track_name", docid );
                //String artist_name = LuceneUtils.getDocno( index, "artist_name", docid );
                String artist_name = LuceneUtils.getDocno( searcher, "artist_name", docid );
                //String album_name = LuceneUtils.getDocno( index, "album_name", docid );
                String album_name = LuceneUtils.getDocno( searcher, "album_name", docid );
